...
Honesty rules (Phase 7 core principles):
  * This repository does NOT ship a trained cabin person model. The active
    estimator is a DEVELOPMENT heuristic (estimator_type="development",
    method="heuristic") and must never be presented as "AI detected passengers".
  * A real-model slot exists (CabinMLPersonDetector) but is used ONLY when a
    model file is configured (FLEETIQ_CABIN_MODEL_PATH) and actually loads.
...
Configuration (env, defaults in brackets):
  ...
  FLEETIQ_CABIN_MODEL_PATH             : unset  (real model NOT available)
...
class CabinMLPersonDetector:
    """Genuine person-detection model wrapper (used ONLY when configured).

    FLEET-IQ does NOT ship a trained cabin model, so this slot is NOT selectable
    by default (FLEETIQ_CABIN_MODEL_PATH unset -> not available). When a
    loadable Ultralytics YOLO weights file is configured it counts detections in
    COCO class 0 ('person') and returns model-native confidence. No model file
    is downloaded, invented, or claimed to exist.
    """
    ...
    def __init__(self, model_path=None):
        self._model_path = model_path or os.environ.get("FLEETIQ_CABIN_MODEL_PATH") or ""
        self._model = None
        self._available = False
        self._error = None
        self._load()
    ...
    def _load(self):
        if not self._model_path or not os.path.isfile(self._model_path):
            self._error = "no configured cabin model file"
            return
        try:
    ...
    def estimate(self, frame, capacity):
        """(count, confidence, model_name) or (None, None, None)."""
        if not self._available or frame is None:
            return None, None, None
        try:
            results = self._model.predict(frame, conf=0.35, verbose=False)[0]
            boxes = results.boxes
...
class CabinOccupancyEngine:
    ...
    def __init__(self, camera_manager, store, mode_state, estimator=None, interval=None):
        ...
        self._real_model = CabinMLPersonDetector()
    ...
    def _tick(self, started=None):
        ...
        if status != "CONNECTED":
            ...
            est = None
            try:
                est = self._estimator.estimate(frame, capacity)
            except Exception as e:  # noqa: BLE001
